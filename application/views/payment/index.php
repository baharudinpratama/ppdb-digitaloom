...
<div class="d-flex flex-column p-3">
    <div class="d-flex flex-column mt-4 mb-1">
        <h1 class="fw-bold" style="font-size: 1.5rem;">Data Pembayaran Seleksi Siswa Baru</h1>
    </div>
    ...
    <div class="d-flex align-items-center gap-4" style="margin-bottom: 4rem;">
        <div class="d-flex flex-fill p-4 rounded-4 bg-white">
            <div class="d-flex align-items-center gap-3">
                <img src="<?= base_url('assets/img/payment/not-pay.png') ?>" alt="not pay" class="rounded-3">
                <div class="d-flex flex-column fw-bold">
                    <p>Peserta yang belum bayar Tes Seleksi</p>
                    <p style="color: #D9AA00;"><span style="font-size: 2rem;"><?= $count_not_paid ?></span> Peserta</p>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-fill p-4 rounded-4 bg-white">
            <div class="d-flex align-items-center gap-3">
                <img src="<?= base_url('assets/img/payment/pay.png') ?>" alt="pay" class="rounded-3">
                <div class="d-flex flex-column fw-bold">
                    <p>Peserta yang sudah bayar Tes Seleksi</p>
                    <p style="color: #43B638;"><span style="font-size: 2rem;"><?= $count_paid ?></span> Peserta</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <table id="table" class="table table-responsive w-100 align-middle">
            <thead>
                <tr>
                    ...
                    <th class="text-center">Tanggal<br>Bayar</th>
                    <th class="text-center">Total<br>Bayar</th>
                ...
                <?php foreach ($students as $student) : ?>
                    <tr>
                        ...
                        <td class="fw-semibold text-center">
                            <div>
                                <?= $student->payment !== null && $student->payment->paid_at !== null ? date('d-m-Y', strtotime($student->payment->paid_at)) : '-' ?>
                            </div>
                            <div class="text-muted small">
                                <?= $student->payment !== null && $student->payment->paid_at !== null ? 'Pkl. ' . date('H:i', strtotime($student->payment->paid_at)) : '' ?>
                            </div>
                        </td>
                        <td class="fw-semibold text-center">
                            <?= $student->payment->status !== 'new' ? 'Rp. ' . $student->payment->amount : 'Rp. 0' ?>
                        </td>
                        <?php
                        $text = '';
                        $color = '';
                        ...
                        <td class="text-center">
                            <?php if ($student->payment->status !== 'confirmed') : ?>
                                <button type="button" data-id="<?= $student->payment->id ?>" class="btn w-100 bg-warning text-white fw-bold btn-confirm">
                                    Konfirmasi
                                </button>
                            <?php endif; ?>
                            <?php if ($student->payment->status === 'confirmed') : ?>
                                <button type="button" data-id="<?= $student->payment->id ?>" class="btn w-100 bg-warning text-white fw-bold btn-confirm" disabled>
                                    Konfirmasi
                                </button>
                            <?php endif; ?>
                        </td>
